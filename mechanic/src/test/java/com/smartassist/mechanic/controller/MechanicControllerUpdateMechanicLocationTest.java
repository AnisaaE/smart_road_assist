...
class MechanicControllerUpdateMechanicLocationTest {
    ...
    @Test
    void updateMechanicLocationShouldReturnUpdatedState() throws Exception {
        when(mechanicService.updateMechanicLocation(eq("mech-1"), any())).thenReturn(
                new MechanicStateResponse("mech-1", MechanicStatus.AVAILABLE, 39.9334, 32.8597));

        mockMvc.perform(put("/mechanics/mech-1/location")
