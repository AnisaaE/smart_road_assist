...
public class DispatcherProxyService {
    ...
    private ResponseEntity<byte[]> forward(HttpServletRequest request, byte[] requestBody, String baseUrl) {
        try {
            ResponseEntity<byte[]> response = restClient.method(HttpMethod.valueOf(request.getMethod()))
                    ...
                    .retrieve()
                    .toEntity(byte[].class);
            return enrichUserResponseIfNeeded(request, sanitizeResponseHeaders(response));
    ...
    private ResponseEntity<byte[]> sanitizeResponseHeaders(ResponseEntity<byte[]> response) {
        HttpHeaders sanitizedHeaders = new HttpHeaders();
        sanitizedHeaders.putAll(response.getHeaders());
        removeHopByHopHeaders(sanitizedHeaders);
    ...
    private void removeHopByHopHeaders(HttpHeaders headers) {
        Set<String> headerNamesToRemove = new LinkedHashSet<>();

        headers.forEach((headerName, headerValues) -> {
            if (HOP_BY_HOP_HEADERS.contains(headerName.toLowerCase(Locale.ROOT))) {
                headerNamesToRemove.add(headerName);
            }

            if (HttpHeaders.CONNECTION.equalsIgnoreCase(headerName)) {
                headerValues.stream()
                        .flatMap(headerValue -> java.util.Arrays.stream(headerValue.split(",")))
                        .map(String::trim)
                        .filter(StringUtils::hasText)
                        .forEach(headerNamesToRemove::add);
            }
        });

        headerNamesToRemove.forEach(headers::remove);
    }
}
